...
OnvifDevice.prototype.setupProxy = function(callback) {
  ...
  var proxyArgs = [];
  proxyArgs.push(this.deviceUrl);
  var parent = cp.fork('./lib/proxy.js', proxyArgs);
...
//TODO: move device-control and presentation route name to predefined variables in cdif

OnvifDevice.prototype.getStreamUri = function(args, callback) {
  var _this = this;
  var options = {};
  // for now we only use default RTP and RTSP stream & transport type
  // if the underlying onvif library has changed default to other value we need to update accordingly
      ...
      _this.getUrlPath(data.uri, function(error, urlObj, path, pathname) {
        if (!error) {
          ...
          if ((args.streamType === 'MJPEG' && args.transport === 'WebSocket') ||
              (args.streamType === '' && args.transport === '')) {
            output.streamUri = 'ws://' + hostIp + ':' + port + '/device-control/' + deviceID + '/presentation' + path + '?type=mediastream';
            callback(null, output);
          } else if (args.streamType === 'RTP-Unicast' && args.transport === 'RTSP') {
            //TODO: if we can host the rtsp reverse proxy then we can consider add this support
            callback(new Error('do not support %s transport and %s stream type', args.transport, args.streamType), null);
          } else {
            callback(new Error('do not support %s transport and %s stream type', args.transport, args.streamType), null);
          }
        } else {
